...
async function applyGoogleLanguage(language, filePath) {
  ...
  const [_, ctrl, shift, swapLanguages] = /[(（]„?(Ctrl|Befehlstaste|Strg|Ktrl|Cmd)\s?\+\s?(Shift|vaihto|Umschalttaste|Maj|Mayús|Maius|Maiús|mayúscula|MAIUSC|Üst Karakter|Skift)\s?\+\s?S\s?.*?[)）].*?jsslot.*?jsslot>(.*?)[(（]/ig.exec(main);
  ...
  const [aboutTitle, languagesTitle, contributeTitle, toolsTitle] = [...about.matchAll(/h-c-header__nav-li-link.*?"\s*title="(.*?)"/g)].map(m => m[1]);
  const [privacyTitle, termsTitle, licensesTitle] = [...about.matchAll(/class="h-c-footer__link"[\s\S]*?>\s*(.*?)\s*</g)].map(m => m[1]).slice(5);
  ...
  const extraForce = {
    ...
    'speech-to-text': {
      start: /jsaction="click:cOuCgd; mousedown:UX7yZ; mouseup:lbsD7e; mouseenter:tfO1Yc; mouseleave:JywGue; touchstart:p6p2H; touchmove:FwuNnf; touchend:yfqBxc; touchcancel:JMtRjd; focus:AHmuwe; blur:O22p3e; contextmenu:mg9Pef;mlnRJb:fLiPzd;" data-disable-idom="true" disabled aria-label="(.*?)"/g.exec(main)[1],
